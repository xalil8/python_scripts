...
def write_data(number_of_image):
    ...
    for i in range(0,number_of_image):
        ...
        resized = cv2.resize(data[0],None,fx=0.5,fy=0.5)
        cv2.imwrite(f"E:/training_dataset_3/test/word_{str(i)}.png", data[0])
        print(f"data {i} writing ...")
        list1.append(data[1])
        list2.append(f"test/word_{str(i)}.png")

    with open("E:/training_dataset_3/gt.txt", "w") as  myfile:
        for m in range(0,number_of_image):
            var1 = list2[m] + " " +list1[m] + "\n"
            myfile.writelines(var1)


    """csv_list = {"filename": list2, "words": list1}
    df = pd.DataFrame(data=csv_list)
    csv_path = 'E:/training_dataset/out.csv'
